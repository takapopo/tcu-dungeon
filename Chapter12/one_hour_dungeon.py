...
def init_battle(): # 戦闘に入る準備をする
    ...
    x=floor//3
    typ = random.randint(x*3, (x+1)*3-1)
    if floor >= 10:
        y=floor&10
        x=y//3
        typ = random.randint(x*3, (x+1)*3-1)
        lev = random.randint(floor-1, floor)
    if floor<=3 :
    ...
    if typ%3==1 :
        emy_lifemax =math.floor(((60*(typ+1) + (lev-1)*10))*random.uniform(1.0,1.25))
        emy_life = emy_lifemax
        emy_str = math.floor((int(emy_lifemax/8))*random.uniform(1.0,1.1))
    if typ%3==2 :
        emy_lifemax =math.floor(((60*(typ+1) + (lev-1)*10))*random.uniform(1.25,1.5))
        emy_life = emy_lifemax
        emy_str = math.floor((int(emy_lifemax/8))*random.uniform(1.1,1.25))
    else:
...
def init_boss_battle() : #ボス戦の準備
    ...
    lev=floor+5
    typ=0
    imgBossEnemy = pygame.image.load("image/kingenemy"+str(typ)+".png")
    boss_emy_name = BOSS_EMY_NAME[typ] + " LV????" 

    emy_lifemax =math.floor(((60*(typ+10) + (lev-1)*10))*random.uniform(1.0,1.25))
    print(emy_lifemax)
    emy_life = emy_lifemax
    emy_str = math.floor((int(emy_lifemax/8))*random.uniform(1.0,1.1)/2)
    print(emy_str)
    emy_x = 440-imgBossEnemy.get_width()/2
    emy_y = 560-imgBossEnemy.get_height()   
...
def draw_battle(bg, fnt): # 戦闘画面の描画
    ...
    if emy_life > 0 and emy_blink%2 == 0:
        bg.blit(imgEnemy, [emy_x, emy_y+emy_step]) #ボス戦時注意
...
def calculateLv ():
    ...
    while exp>=expToNextLv :
        count1+=1
        expToNextLv = round(((currentLv + count1) * (currentLv + count1) * 9) / 2)
    preLv=currentLv
    count0=count1
    currentLv +=(count1)
...
def main(): # メイン処理
    ...
    se = [ # 効果音とジングル
        ...
        pygame.mixer.Sound("sound/勝利1.ogg"),
        pygame.mixer.Sound("sound/se1.ogg"),
        pygame.mixer.Sound("sound/切り裂き1.ogg"),
        pygame.mixer.Sound("sound/切り裂き2.ogg"),
        pygame.mixer.Sound("sound/切り裂き3.ogg")
    ]
    ...
    while True:
        ...
        if idx == 0: # タイトル画面
            if tmr == 1:
                pygame.mixer.music.load("sound/tittle1.ogg")
            ...
            if tmr == 1:
                ...
                set_message("Level"+str(currentLv))
                se[4].play()

            if tmr == 21:
                set_message("Max life + "+str(lif_p*count1))
                pl_lifemax = pl_lifemax + lif_p**count1
                pl_life=pl_lifemax
            if tmr == 26:
                set_message("Str + "+str(str_p*count1))
                pl_str = pl_str + str_p**count1
            if tmr == 50:
                idx = 22
